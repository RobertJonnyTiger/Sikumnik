...
// import { AnalogyBlock } from "./AnalogyBlock"; // Note: AnalogyBlock dependency. For now, assuming standard one exists or simplified.
        ...
        return lines.map((line, lineIdx) => {
            ...
            const content = cleanedLine.split(BOLD_TAG_REGEX).map((part, i) => {
                if (part.startsWith("<b>") && part.endsWith("</b>")) {
                    ...
                    return (
                        <TermTooltip key={i} term={term} definition="מושג חשוב.">
            ...
            if (numberedMatch) {
                return (
                    <div key={lineIdx} className="flex gap-6 items-start mb-8 group/list hover:translate-x-[-8px] transition-transform duration-500">
                        ...
                        <div className="flex-1 text-lg md:text-xl leading-relaxed font-main text-foreground/90">
            ...
            return <div key={lineIdx} className="mb-4 text-lg md:text-xl leading-relaxed font-main">{content}</div>;
        });
    };
    ...
    return (
        <section className="space-y-8 group scroll-mt-32">
            <header className="flex flex-col gap-6">
                ...
                <h3 className="text-3xl md:text-4xl font-black text-white tracking-tight leading-tight font-main -mt-12 relative z-10 px-2">
                    {cleanTitle}
                </h3>
            </header>

            <div className="text-lg md:text-xl font-medium text-foreground/90 leading-tight pr-10 border-r-8 border-primary/20 py-6 font-main">
                {renderAcademicText()}
            </div>

            {/* If analogy text exists, simple render (avoiding AnalogyBlock missing import for now unless we move it too) */}
            {showAnalogy && analogyText && (
                <div className="mt-8 p-6 bg-accent/5 border-r-4 border-accent rounded-l-2xl">
                    <p className="text-xl text-accent/90 italic">{cleanText(analogyText)}</p>
                </div>
            )}
        </section>
    );
