...
export const CommonMistakes: React.FC<CommonMistakesProps> = ({ mistakes, introText, className }) => {
    return (
        <div className={cn("relative group", className)}>
            <div className="absolute -inset-1 bg-gradient-to-r from-rose-500/10 to-orange-500/10 rounded-[2.5rem] blur opacity-25 group-hover:opacity-40 transition duration-1000 group-hover:duration-200" />
                ...
                <div className="relative z-10">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        {mistakes.map((item, idx) => (
                            <div key={idx} className="flex flex-col gap-4 p-6 bg-white/[0.02] border border-white/[0.05] rounded-2xl hover:bg-white/[0.04] transition-all">
