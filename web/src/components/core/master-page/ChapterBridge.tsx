...
export const ChapterBridge: React.FC<ChapterBridgeProps> = ({ nextChapter, prevChapter, className }) => {
    return (
        <div className={cn("grid grid-cols-1 md:grid-cols-2 gap-6 mt-12", className)}>
            {/* PREVIOUS CHAPTER */}
            {prevChapter ? (
                <Link
                    href={`/courses/accounting/${prevChapter.id}`}
            ...
            {nextChapter ? (
                <Link
                    href={nextChapter.isLocked ? "#" : `/courses/accounting/${nextChapter.id}`}
                    className={cn(
                        ...
                        nextChapter.isLocked
                            ...
                            : "bg-gradient-to-br from-teal-900/20 to-slate-900/50 border-teal-500/30 hover:border-teal-500/50 hover:shadow-lg hover:shadow-teal-500/5"
                    )}
                >
