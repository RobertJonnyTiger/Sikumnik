...
export const DefinitionBlock: React.FC<DefinitionBlockProps> = ({ title, term, definition, definitions = [], className }) => {
    ...
    return (
        <div className={cn("relative group", className)}>
            <div className="absolute -inset-1 bg-gradient-to-r from-blue-500/10 to-teal-500/10 rounded-[2.5rem] blur opacity-25 group-hover:opacity-40 transition duration-1000 group-hover:duration-200" />

            <div className="relative bg-slate-950/40 backdrop-blur-xl border border-white/5 rounded-[2rem] p-8 md:p-10 shadow-2xl overflow-hidden">
                ...
                <div className="relative z-10">
                    ...
                    <div className="space-y-8">
                        {allDefinitions.map((def, idx) => (
                            <div key={idx}>
                                {def.term && (
                                    <h4 className="text-xl md:text-2xl font-bold text-teal-400 mb-3 font-assistant">
                                        {def.term}
                                    </h4>
                                )}
                                <p className="text-lg md:text-xl text-slate-300 leading-relaxed max-w-3xl font-assistant">
                                    {def.definition}
                                </p>
                            </div>
                        ))}
